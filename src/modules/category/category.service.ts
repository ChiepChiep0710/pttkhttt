...
// import { Categories, CategoriesSchema } from '../../database/models/Category.model';
// import { Product, ProductSchema } from '../../database/models/product.model';
import { CategoryDto, CategoryUpdateDto, DeleteCategoryDto } from './category.dto';
...
export class CategoryService {
    ...
    async insertCategory(data: CategoryDto, request) {
    ...
    async deleteCategory(id: DeleteCategoryDto, request) {
        const admin = Utils.decodeJwtService(request.headers['authorization'], this.jwtService);
        if (admin['role'] !== 'admin') throw new ApiError('You done have permission to do this action', "E3");
        try {
            await this.CategoryModel.deleteMany({ "_id": { $in: id.deleteId } })
            return new ApiOK({ result: true })
        } catch (err) {
            return new ApiError(err.message)
        }
    ...
    async updateCategory(data: CategoryUpdateDto, request) {
        const admin = Utils.decodeJwtService(request.headers['authorization'], this.jwtService);
        if (admin['role'] !== 'admin') throw new ApiError('You done have permission to do this action', "E3");
    ...
    async searchCategory(data: any) {
        ...
        console.log({ offset, limit, sortField, sortType })
        try {
            let total = await this.CategoryModel.find(query).countDocuments();
            let searchres = await this.CategoryModel.find(query)
                .skip(offset)
                .limit(limit)
